...
Everything is organized into the following folders:
...
- `notebooks`: Jupyter notebook(s) containing visualizations and analysis
- `src`: all python code for data processing, analysis, and visualization

## Running the Code

All code can be run from the Jupyter notebook(s) in the `notebooks` folder. Before running the code, make sure to install all required third party modules listed in `requirements.txt`.
